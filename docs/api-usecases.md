...
- 操作:
  ...
  - 再試行
- UX:
  - HomeのToday専用セクションは表示しない
  - Homeヘッダー下に期間テキストは表示しない
  - 初期ロードはスケルトン表示を優先する
...
- 主画面:
  - `Home` の Bottom Sheet
- UX:
  - 通常は Bottom Sheet で `date` / `quantity` / `optionalData`（任意メモ）を入力
...
- UX要件:
  ...
  - `optionalData` も更新対象とし、未入力時は未設定として扱う
...
- UX:
  ...
  - 記録一覧では `optionalData` がある行のみ要約を表示する
  - 取得結果から統計（合計・記録日数・最大・平均(記録日)・現在連続日数・最長連続日数）を算出して表示する
  - 取得失敗時は再試行導線を表示する
...
- `503 && isRejected=true`（Pixela無料枠の一時拒否）は共通クライアントで短間隔リトライして吸収する
- UIでは再試行導線を必ず表示
- 認証情報不足時は `認証ハブ` に戻す
